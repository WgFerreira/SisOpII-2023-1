...
void *datagram::replicate_sender(Station *station, ReplicateMessageQueue *send_queue)
{
  int sockfd = datagram::open_socket();
  bool running = true;
  ...
  while (running)
  {
    ...
    while (!send_queue->queue.empty())
    {
      struct replicate_message msg = send_queue->pop();

      struct sockaddr_in sock_addr = socket_replicate_address(msg.address);
  ...
  close(sockfd);
  if (station->debug)
    std::cout << "replicate sender: saindo" << std::endl;
  return 0;
}
...
void *datagram::replicate_receiver(Station *station, ReplicateMessageQueue *replicate_queue)
{
  int sockfd = datagram::open_socket();
  
  struct sockaddr_in bound_addr = socket_replicate_address(INADDR_ANY);
  if (bind(sockfd, (struct sockaddr *) &bound_addr, sizeof(struct sockaddr)) < 0) 
      std::cerr << "ERROR binding socket" << std::endl;
  ...
  {
    struct sockaddr_in client_addr;
    struct table_packet client_data;
    socklen_t client_addr_len = sizeof(struct sockaddr_in);

    int n = recvfrom(sockfd, &client_data, sizeof(struct table_packet), 0, (struct sockaddr *) &client_addr, &client_addr_len);
    if (n > 0)
    {
      //if (client_data.station.pid == station->getPid())
      //  continue;
        
      std::cout << "replicate received" << std::endl;
        
      // inet_ntop(AF_INET, &(client_addr.sin_addr), client_data.station.ipAddress, INET_ADDRSTRLEN);
      // StationTable station_table = StationTable::deserialize(client_data.station_table);
      /*
      if (station->debug)
        std::cout << "a replicate message was received " << messageTypeToString(client_data.type) << std::endl;
      
      struct replicate_message msg;
      msg.type = client_data.type;
      msg.address = client_addr.sin_addr.s_addr;
      msg.payload = *(client_data.station_table.table);
      msg.sequence = client_data.seqn;
  ...
  close(sockfd);
  if (station->debug)
    std::cout << "replicate receiver: saindo" << std::endl;
  return 0;
}
...
struct sockaddr_in datagram::socket_replicate_address(in_addr_t addr)
{
  struct sockaddr_in address;
  address.sin_family = AF_INET;     
  address.sin_port = htons(REPLICATE_PORT);
  address.sin_addr.s_addr = addr;
  memset(&(address.sin_zero), 0, 8);
  return address;
}
