...
auto *send_replicate_queue = new ReplicateMessageQueue();
auto *discovery_queue = new MessageQueue();
auto *monitor_queue = new MessageQueue();
auto *replicate_queue = new ReplicateMessageQueue();
...
int main(int argc, const char *argv[]) {
    ...
    auto th_replicate_sender = std::thread(&datagram::replicate_sender, station, send_replicate_queue);
    auto th_replicate_receiver = std::thread(&datagram::replicate_receiver, station, replicate_queue);
    ...
    auto th_replicate = std::thread(&replicate::replicate, station, manage_queue, replicate_queue, stationTable);
    ...
    th_interface.join();
    th_command.join();
    th_replicate_sender.join();
    th_replicate_receiver.join();
    th_replicate.join();
