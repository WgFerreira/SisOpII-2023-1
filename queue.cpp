...
void ReplicateMessageQueue::push(replicate_message m) 
{
  this->mutex_write.lock();
    this->queue.push_back(m);
  this->mutex_write.unlock();
...
void ReplicateMessageQueue::push(list<replicate_message> m) 
{
  this->mutex_write.lock();
    this->queue.splice(this->queue.end(), m);
  this->mutex_write.unlock();
...
replicate_message ReplicateMessageQueue::pop() 
{
  this->mutex_write.lock();
    replicate_message m = this->queue.front();
    this->queue.pop_front();
  this->mutex_write.unlock();
